...
suspend fun loadDataRecords(context: Context, chipId: String, from: Long, to: Long): ArrayList<com.chillibits.pmapp.model.DataRecord>? {
    try{
        ...
        if(response.status == HttpStatusCode.OK) {
            val records = ArrayList(Json.parse(com.chillibits.pmapp.model.DataRecordCompressedList.serializer(), response.readText()).items.map {
