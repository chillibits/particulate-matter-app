...
class DiagramActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mode == MODE_SENSOR_DATA) {
            ...
            // Prepare data
            SensorActivity.sort_mode = SensorActivity.SORT_MODE_TIME_ASC
            for (current_records in compareRecords) Collections.sort(current_records)
        }
        ...
        try {
            ...
            if (mode == MODE_SENSOR_DATA) {
                ...
                first_time = records[0].dateTime.time
                xAxis.valueFormatter = TimeFormatter(first_time)
                for (r in records) {
                    if (show1) entries1.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toDouble(), r.p1, "µg/m³"))
                    if (show2) entries2.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toDouble(), r.p2, "µg/m³"))
                    if (show3) entries3.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toDouble(), r.temp, "°C"))
                    if (show4) entries4.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toDouble(), r.humidity, "%"))
                    if (show5) entries5.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toDouble(), r.pressure, "hPa"))
                }
                ...
                if (show1 && (enableAverage || enableMedian)) dataSets.add(getAverageMedianPM1(enableAverage, enableMedian, first_time))
                if (show2) dataSets.add(p2)
                if (show2 && (enableAverage || enableMedian)) dataSets.add(getAverageMedianPM2(enableAverage, enableMedian, first_time))
                if (show3) dataSets.add(temp)
                if (show3 && (enableAverage || enableMedian)) dataSets.add(getAverageMedianTemperature(enableAverage, enableMedian, first_time))
                if (show4) dataSets.add(humidity)
                if (show4 && (enableAverage || enableMedian)) dataSets.add(getAverageMedianHumidity(enableAverage, enableMedian, first_time))
                if (show5) dataSets.add(pressure)
                if (show5 && (enableAverage || enableMedian)) dataSets.add(getAverageMedianPressure(enableAverage, enableMedian, first_time))
                if ((show1 || show2) && (enableThresholdEu || enableThresholdWho)) {
                    dataSets.add(getThresholdPM1(enableThresholdEu, enableThresholdWho, first_time))
                    dataSets.add(getThresholdPM2(enableThresholdEu, enableThresholdWho, first_time))
                }
                chart.data = LineData(dataSets)
            } else if (mode == MODE_COMPARE_DATA) {
                // Get first time
                first_time = java.lang.Long.MAX_VALUE
                for (i in compareSensors.indices) {
                    try {
                        val current_first_time = compareRecords[i][0].dateTime.time
                        first_time = if (current_first_time < first_time) current_first_time else first_time
                    } catch (e: Exception) {}
                }
                xAxis.valueFormatter = TimeFormatter(first_time)
                // Plot data
                ...
                for (i in compareSensors.indices) {
                    if (compareRecords[i].size > 0) {
                        ...
                        for (r in compareRecords[i]) {
                            try {
                                if (show1) {
                                    entries.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toFloat().toDouble(), r.p1.toFloat().toDouble(), "µg/m³"))
                                } else if (show2) {
                                    entries.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toFloat().toDouble(), r.p2.toFloat().toDouble(), "µg/m³"))
                                } else if (show3) {
                                    entries.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toFloat().toDouble(), r.temp.toFloat().toDouble(), "°C"))
                                } else if (show4) {
                                    entries.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toFloat().toDouble(), r.humidity.toFloat().toDouble(), "%"))
                                } else if (show5) {
                                    entries.add(DiagramEntry(((r.dateTime.time - first_time) / 1000).toFloat().toDouble(), r.pressure.toFloat().toDouble(), "hPa"))
                                }
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
            ...
            chart.marker = DiagramMarkerView(this, R.layout.diagram_marker_view, first_time)
            // Customize legend
    ...
    private fun getThresholdPM2(enable_eu_thresholds: Boolean, enable_who_thresholds: Boolean, first_timestamp: Long): LineDataSet {
        val th_entries: MutableList<Entry>
        th_entries = ArrayList()
        if (enable_eu_thresholds) {
            th_entries.add(Entry(((records[0].dateTime.time - first_timestamp) / 1000).toFloat(), Constants.THRESHOLD_EU_PM2_5.toFloat()))
            th_entries.add(Entry(((records[records.size - 1].dateTime.time - first_timestamp) / 1000).toFloat(), Constants.THRESHOLD_EU_PM2_5.toFloat()))
        } else if (enable_who_thresholds) {
            th_entries.add(Entry(((records[0].dateTime.time - first_timestamp) / 1000).toFloat(), Constants.THRESHOLD_WHO_PM2_5.toFloat()))
            th_entries.add(Entry(((records[records.size - 1].dateTime.time - first_timestamp) / 1000).toFloat(), Constants.THRESHOLD_WHO_PM2_5.toFloat()))
        }
